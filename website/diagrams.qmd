...
- C3: `maps_t`
  ...
  * __`destroy_maps` Pre/Post Conditions:__
    ...
    * _Postconditions:_
      1. Memory allocated by `maps_t` and the underlying hashmaps is freed and returned to the system for assigning to
         other objects
- C4 `fraction_t`
    ...
    * _Postconditions:_
      1. Returns string comparison of `name` elements in `target_t`
        ...
        3. Returns <0 if first non-matching character of first string is less than the corresponding character in the
           second string
- C6: `output_names_t`
  * __Description:` Combines names of all output files into a single struct
  ...
  * __`destroy_output_names` Pre/Post Conditions:__
    ...
    * _Postconditions:_
      1. Memory allocated by `output_names_t` and the underlying character array pointers is freed and returned to the
